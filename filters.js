...
// ===== 災害支援情報を「見ない」モード =====
// 「災害支援情報を見ない」ON時に非表示にする災害・救助・支援系キーワード（完全ローカル）。
// 旧「見る」モードの許可リストは 共有/拡散/お知らせ など汎用語を含んでいたため、
// 非表示用に「災害イベント語＋救助・支援の具体語」へ絞り、誤爆を抑えている。
const DISASTER_SIGNAL = /(?:地震|余震|震度|震源地|マグニチュード|津波|震災|災害|台風|豪雨|洪水|河川氾濫|土砂災害|土砂崩れ|地滑り|噴火|竜巻|高潮|避難|避難所|避難指示|避難場所|避難所開設|安否|安否確認|被災|被災地|被災者|救助|救助隊|救出|救難|救援|支援物資|支援金|義援金|被災者支援|募金|寄付|ボランティア|復旧|復興|停電|断水|給水|給油|炊き出し|罹災|地震速報|緊急地震速報|救助要請|earthquake|aftershock|tsunami|evacuation|evacuate|shelter|epicenter|magnitude|flood|landslide|mudslide|typhoon|rescue|relief|donation|donate|volunteer|blackout|power outage|disaster|wildfire)/i;
// 感情をえぐるグラフィック表現（災害支援情報を見ないモードでも非表示を維持）
...
// ===== グルメ情報を「見ない」モード =====
// 「グルメ情報を見ない」ON時に非表示にする食べ物・食事系キーワード（完全ローカル）。
// 素材名（野菜・卵など）や「そば（=そばに）」「パン（=パンツ）」「カレー（人名）」のような
// 誤爆しやすい語は除外し、食事・料理・外食・レシピ系に絞っている。
const FOOD_SIGNAL = /(?:ご飯|ごはん|朝ごはん|昼ごはん|夜ごはん|朝食|昼食|夕食|食事|食べ物|食べもの|グルメ|料理|お料理|レシピ|クッキング|ラーメン|うどん|寿司|すし|ピザ|ハンバーガー|ハンバーグ|ケーキ|スイーツ|お菓子|デザート|コーヒー|カフェ|喫茶店|レストラン|居酒屋|焼肉|お好み焼き|たこ焼き|弁当|お弁当|パスタ|サラダ|スープ|味噌汁|おでん|鍋|定食|丼|天ぷら|刺身|焼き魚|焼き鳥|ステーキ|食べたい|お腹すいた|お腹がすいた|腹ペコ|空腹|満腹|もぐもぐ|ぱくぱく|food|meal|meals|delicious|yummy|tasty|cooking|recipe|recipes|restaurant|breakfast|lunch|dinner|snack|snacks|pizza|ramen|sushi|cake|dessert|coffee|hungry|starving)/i;

// ===== AI驚き屋を「見ない」モード =====
// AIの能力・脅威を誇張して不安や驚きを煽る投稿（AI驚き屋）を非表示にするキーワード（完全ローカル）。
// 「AI＝全部」ではなく、AGI終末論・シンギュラリティ・人類滅亡・全仕事喪失などの煽りに限定している。
